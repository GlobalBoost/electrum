...
from electrum_bsty.storage import WalletStorage, StorageReadWriteError
from electrum_bsty.wallet import Wallet, InternalAddressCorruption
from electrum_bsty.util import profiler, InvalidPassword, send_exception_to_crash_reporter
from electrum_bsty.plugin import run_hook
from electrum_bsty.util import format_satoshis, format_satoshis_plain, format_fee_satoshis
from electrum_bsty.util import PR_UNPAID, PR_PAID, PR_EXPIRED, PR_FAILED, PR_INFLIGHT
...
class ElectrumWindow(App):
    ...
    def on_new_intent(self, intent):
        ...
        if intent.getScheme() == 'globalboost':
            self.set_URI(data)
    ...
    def on_qr(self, data):
        from electrum_bsty.bitcoin import base_decode, is_address
        data = data.strip()
        if is_address(data):
            self.set_URI(data)
            return
        ...
        from electrum_bsty.transaction import Transaction
        from electrum_bsty.util import bh2u
        try:
    ...
    def init_ui(self):
        ...
        self.icon = "electrum_bsty/gui/icons/electrum-bsty.png"
    ...
    def get_max_amount(self):
        from electrum_bsty.transaction import TxOutput
